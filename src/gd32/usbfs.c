...
// The classic controller exposes one endpoint register per four-byte slot.
// Word accesses match the working single-buffer path on GD32F303.
#define USB_EPR(ep) (((volatile uint32_t *)USB_BASE)[ep])
...
// Put an endpoint register into an exact known state. The status and DTOG
// fields toggle when software writes a one, so assigning a constant does not
// necessarily clear them after a second SET_CONFIGURATION or bus reset.
static void
epr_reset_config(uint32_t ep, uint32_t value)
{
    uint32_t epr = USB_EPR(ep);
    USB_EPR(ep) = calc_epr_bits(
        epr, EPR_TBITS | EPR_RWBITS | EPR_RWCBITS, value);
}
...
usb_reset(void)
{
    uint32_t ep = 0;
    epr_reset_config(
        ep, USB_EP_CONTROL | USB_EP_RX_VALID | USB_EP_TX_NAK);

    ep = USB_CDC_EP_ACM;
    epr_reset_config(ep, USB_CDC_EP_ACM | USB_EP_INTERRUPT
                     | USB_EP_RX_NAK | USB_EP_TX_NAK);

    ep = USB_CDC_EP_BULK_OUT;
    epr_reset_config(ep, USB_CDC_EP_BULK_OUT | USB_EP_BULK | USB_EP_KIND
...
usb_init(void)
{
    ...
    // Enable USB clock
    // 120MHz PLL / 2.5 provides the required 48MHz USB clock.
    RCU_CFG0 &= ~(RCU_CFG0_USBDPSC | RCU_CFG0_USBDPSC_2);
    RCU_CFG0 |= RCU_CKUSB_CKPLL_DIV2_5;
    enable_pclock(RCU_USBD);
