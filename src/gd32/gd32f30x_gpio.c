...
gpio_out_reset(struct gpio_out g, uint8_t val)
{
	...
	// Preload the output latch before enabling output mode to avoid glitches.
	if(!val)
	{
...
{
    // Only the I2C0 remap is currently exposed by the GD32 platform.  Avoid
    // the vendor helper's broad masks, which may overwrite SWJ configuration.
    if (gpio_remap != GPIO_I2C0_REMAP)
        shutdown("Unsupported GPIO remap");
    if (newvalue != DISABLE)
