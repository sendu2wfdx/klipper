...
struct gpio_adc gpio_adc_setup(uint8_t pin)
{
    ...
    // Allow the ADC analog circuitry to stabilize before calibration.
    udelay(10);
    ...
    while (_ADC_CTL1 & ADC_CTL1_RSTCLB) {
        if (!timer_is_before(timer_read_time(), timeout))
            shutdown("ADC calibration reset timeout");
    }

    _ADC_CTL1 |= ADC_CTL1_CLB; //calibration_en
    timeout = timer_read_time() + timer_from_us(5000);
...
gpio_adc_sample(struct gpio_adc g)
{
    ...
    // The on-chip sensor is ADC_IN16 and requires at least 17.1us of
    // acquisition time.  Keep the short sample time for external channels.
    uint32_t sample_time = g.chan == 16
        ? ADC_SAMPLETIME_239POINT5 : ADC_SAMPLETIME_7POINT5;
...
delaytime:
    ...
    return timer_from_us(g.chan == 16 ? 30 : 20);
}
