...
struct io_remap {
    ...
    uint8_t oid, source_pullup, default_value, filter_count, active_count;
    uint8_t enabled;
};
...
io_remap_event(struct timer *timer)
{
    ...
    uint8_t active = gpio_in_read(ir->source) == !ir->source_pullup;
    if (active) {
...
command_config_io_remap(uint32_t *args)
{
    struct io_remap *ir = oid_alloc(args[0], command_config_io_remap,
                                    sizeof(*ir));
    ir->oid = args[0];
    ir->source_pullup = !!args[2];
    ir->source = gpio_in_setup(args[1], ir->source_pullup);
