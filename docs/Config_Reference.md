...
This downstream sensor backend supports both `[load_cell]` and
`[load_cell_probe]` using the normal Klipper force/probe architecture.
...
Both pins must be on the same MCU. `sample_rate` is 10, 40, 640, or 1280;
`gain` is 1, 2, 64, or 128. DOUT is bidirectional during configuration.
See [CS1237 and GPIO forwarding](CS1237_GPIO_Forward.md) for F009 wiring and
the independent `[gpio_forward name]` extension. No PRTouch logic is used.
